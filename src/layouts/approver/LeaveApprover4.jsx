  ...
  const handleApprove = async (leaveRequestId) => {
    ...
    if (formValues) {
      try {
        ...
        await axios.patch(
          apiEndpoints.ApproveleaveRequestsByFouthApprover(leaveRequestId),
          formValues,
          {
            headers: {
  ...
  const handleReject = async (leaveRequestId) => {
    ...
    if (reason) {
      try {
        const token = localStorage.getItem("token");
        await axios.patch(
          apiEndpoints.RejectleaveRequestsByFouthApprover(leaveRequestId),
          { reason },
          {
