  ...
  const handleApprove = async (leaveRequestId) => {
    const { value: formValues } = await Swal.fire({
      ...
      html:
        ...
        `<textarea id="comment" class="swal2-textarea" placeholder="ความคิดเห็นเพิ่มเติม"></textarea><br/>`+
        `<label for="documentNumber">เลขที่:</label>` +
        `<textarea id="documentNumber" class="swal2-textarea" placeholder="ระบุเลขที่เอกสาร"></textarea>`,
      focusConfirm: false,
      ...
      preConfirm: () => {
        ...
        if (!remarks || !comment ||!documentNumber) {
          Swal.showValidationMessage("กรุณากรอกเหตุผลและความคิดเห็นให้ครบถ้วน");
          return;
        }
        return { remarks, comment, documentNumber };
      },
    });
